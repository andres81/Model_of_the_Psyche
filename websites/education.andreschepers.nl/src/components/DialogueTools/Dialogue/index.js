...
import {FaTimes, FaUniversity, FaPlay, FaStop, FaPause} from 'react-icons/fa';
    ...
    getControls = () => {
        ...
        return <div className="dialogue-controls row text-start">
            <div className="col-sm-6 dialogue-controls-inner">
                ...
                { showFileControls && this.props.showLoadSaveButtons && <AiTwotoneFolderOpen className="dialogue-controls-load-button" onClick={() => this.fileInputRef.current.click()} /> }
                { showFileControls && this.props.showLoadSaveButtons && <AiOutlineDownload className="dialogue-controls-download-button" onClick={() => downloadJson(this.state.dialogueJson.title, this.state.dialogueJson)} /> }
    ...
    createAudiobutton = () => {
        if (this.state.dialogueJson && !!this.state.dialogueJson.audioSource) {
            return <span className="dialogue-audio-play-button">
                ...
                &nbsp;&nbsp;
                {!this.state.isAudioPlaying && <FaPlay onClick={this.audioPlay} />}
                {this.state.isAudioPlaying && <FaPause onClick={this.audioPaused} />}
            </span>
        }
    }
